...
def action_array_index_to_coord(idx:int, # index into 1D array
                                ...
                                ncols:int=None): # type: ignore # board columns. Inferred from `nrows` if `None`
    """
...
class LibertyTracker():
    ...
    def __init__(self, 
                 group_index:np.ndarray=None, # type: ignore # an NxN numpy array of group_ids. `-1`: no group.
                 groups:dict=None, # type: ignore # a `dict` of `group_id:groups`
                 liberty_cache:np.ndarray=None, # type: ignore # an NxN numpy array of liberty counts.
                 max_group_id:int=1):
        """
...
class Position():
    def __init__(self, 
                 board:np.ndarray=None, # type: ignore # numpy array
                 ...
                 lib_tracker:LibertyTracker=None, # type: ignore # `LibertyTracker` object
                 ko:int=None, # type: ignore # a `Move`
                 recent:tuple=tuple(), # tuple of `PlayerMoves`; `recent[-1]` is the last move
                 board_deltas:np.ndarray=None, # type: ignore # numpy array of shape `(n, N, N)` representing changes made to the board at each move (played move and captures). Should satisfy `next_pos.board - next_pos.board_deltas[0] == pos.board`
...
def get_history(state):
    """
    Reconstructs the last 8 timesteps of game history from Go board deltas.

    Returns
    -------
    out: ndarray
        (17,19,19) integer array of the last 8 turns. Axis-0[:8]: timestep's current player positions, 
        Axis-0[8:16]: timestep's opposing player positions, Axis-0[16]: player turn.

    Notes
    -----
    Adapted from https://github.com/tensorflow/minigo/blob/master/features.py#L58 under Apache 2.0 license.

    "Current player" is relative to timestep and not necessarily the "host" player. History states 
    represent alternating players going back in time.
    """
    color_turn = np.zeros((1, N, N), dtype=np.uint8)
    color_turn = color_turn + 1 if state.to_play == 1 else color_turn

    # simipler to calculate with the axis-0 being the 16 board states, then roll axis 0 to the end
    history = np.zeros((16, N, N), dtype=np.uint8)

    num_deltas_avail = state.board_deltas.shape[0]
    cumulative_deltas = np.cumsum(state.board_deltas, axis=0)
    last_eight = np.tile(state.board, [8,1,1])
    # apply deltas to compute previous board states
    last_eight[1:num_deltas_avail + 1] -= cumulative_deltas
    # if no more deltas available, repeat oldest board
    last_eight[num_deltas_avail + 1:] = last_eight[num_deltas_avail].reshape(1,N,N)

    history[::2] = last_eight == state.to_play # every other state set to match player turn
    history[1::2] = last_eight == -state.to_play

    history = np.concatenate((history, color_turn), axis=0)
    # np.rollaxis(history, 0, 3) # converts to H,W,C (row, col, depth) order for TensorFlow; for PyTorch leave as C,H,W
    return history